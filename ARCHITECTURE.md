...
- **HTTP** : `GET /` sert le client buildé ; `GET /health` pour la plateforme ; `GET /scores` renvoie le top 20 du classement général (`[{name, wins, games}]`) ; upgrade WebSocket sur `/ws`. Un seul port.
