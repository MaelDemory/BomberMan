...
| Data/storage | Rooms en mémoire + `scores.json` sur volume Fly (1 Go, ~0,15 €/mois) | Parties éphémères ; seul le classement général doit survivre aux redémarrages — un fichier JSON atomique suffit à cette échelle | Redis/Postgres (surdimensionnés), SQLite (rien à requêter) |
...
- **Protocole (JSON sur WebSocket, v1)** — source de vérité : `packages/shared/src/protocol/messages.ts` :
  - Client → serveur : `{type:"create", name}`, `{type:"join", roomCode, name}`, `{type:"start"}` (hôte uniquement), `{type:"input", tick, keys}` (état des touches daté du tick client, un par tick de prédiction), `{type:"addBot", difficulty}` / `{type:"removeBot", botId}` / `{type:"setMap", map}` (hôte, en lobby), `{type:"leave"}`.
...
- **Rooms** : code 4 lettres généré serveur, 2-4 joueurs, démarrage par l'hôte, room détruite quand vide. Serveur = source de vérité unique ; le client n'envoie que des inputs. L'hôte choisit la map (`maps.ts` : Classique/Ouverte/Couloirs/Chaos ou aléatoire), diffusée dans `joined`/`lobby`.
- **Mort subite** : à partir du tick 1800 (90 s), un mur indestructible tombe toutes les 0,5 s en spirale horaire depuis le bord (`suddendeath.ts`), écrasant blocs, bombes, bonus et joueurs (chevauchement de boîte). Entièrement déterministe (aucun PRNG) : le client affiche les prochaines cases condamnées et le compte à rebours, les bots fuient les cases à venir. Durée max d'une partie ≈ 2 min 40.
- **Bots (mode solo ou parties mixtes)** : joueurs normaux de la sim dont les inputs sont calculés côté serveur à chaque tick par `packages/shared/src/bomberman/bot.ts` (carte de danger, BFS, pose seulement si repli possible ; la difficulté module cadence de décision, réaction au danger, chasse et recharge). L'IA vit hors de `step()` et hors de `GameState` : le déterminisme de la sim n'est pas affecté. Le seuil de démarrage (≥ 2 joueurs) compte les bots ; une room sans humain est détruite.
...
- **HTTP** : `GET /` sert le client buildé ; `GET /<CODE>` (4 lettres) sert aussi l'app — lien d'invitation, le client préremplit le code depuis l'URL ; `GET /health` pour la plateforme ; `GET /scores` renvoie le top 20 du classement général (`[{name, wins, games}]`) ; upgrade WebSocket sur `/ws`. Un seul port.
- **Scores (classement général)** : fichier `scores.json` sur disque persistant (`DATA_DIR`, volume Fly en prod), écrit de façon atomique à chaque fin de partie. Une partie compte si ≥ 2 humains y participaient (bots tolérés) ; identité = pseudo insensible à la casse, **sans authentification — spoofable, assumé**. Une victoire de bot ne crédite personne.
