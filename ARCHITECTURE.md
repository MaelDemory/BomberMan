...
- **Protocole (JSON sur WebSocket, v1)** — source de vérité : `packages/shared/src/protocol/messages.ts` :
  - Client → serveur : `{type:"create", name}`, `{type:"join", roomCode, name}`, `{type:"start"}` (hôte uniquement), `{type:"input", tick, keys}` (état des touches daté du tick client, un par tick de prédiction), `{type:"addBot", difficulty}` / `{type:"removeBot", botId}` (hôte, en lobby), `{type:"leave"}`.
